...
public class StoreDao {
	...
	public void insert(StoreDto storeDto, MultipartFile attach) throws IllegalStateException, IOException {
		...
		Object[] data = { storeDto.getStoreName(), storeDto.getStorePost(), storeDto.getStoreAddress1(),
				storeDto.getStoreAddress2(), storeDto.getStoreCategory(), storeDto.getStoreType(),
				storeDto.getStoreContact(), storeDto.getStoreIntro(), storeDto.getStoreDtip(),
				storeDto.getStoreMinprice(), storeDto.getStoreOpenHour(), storeDto.getStoreCloseHour(),
	...
	public boolean update(StoreDto storeDto) {
		String sql = "UPDATE store SET store_name=?, store_post=?, store_address1=?, store_address2=?, "
				...
				+ "store_delivery=?, store_closed=?, store_update=?, store_business_number=?" + "WHERE store_no=?";

		Object[] data = { storeDto.getStoreName(), storeDto.getStorePost(), storeDto.getStoreAddress1(),
				storeDto.getStoreAddress2(), storeDto.getStoreCategory(), storeDto.getStoreType(),
				storeDto.getStoreContact(), storeDto.getStoreIntro(), storeDto.getStoreDtip(),
				storeDto.getStoreMinprice(), storeDto.getStoreOpenHour(), storeDto.getStoreCloseHour(),
				storeDto.getStoreDelivery(), storeDto.getStoreClosed(), storeDto.getStoreUpdate(),
				storeDto.getStoreBusinessNumber(), storeDto.getStoreNo() };
...
//  	}
	...
	public List<StoreDto> searchStoresByMenuName(String menuName) {
		...
		String sql = "SELECT DISTINCT s.* " + "FROM store s " + "INNER JOIN menu m ON s.store_no = m.store_no "
				+ "WHERE INSTR(UPPER(m.menu_name), UPPER(?)) > 0 " + "ORDER BY s.store_no ASC";
	...
	//찜 목록
	@Autowired
	private StoreLikeMapper storeLikeMapper;
	public List<StoreDto> selectLikeStore(String memberId){
		 String sql = "SELECT * FROM store_like WHERE member_id = ?";
		 Object[] data = {memberId};
	         List<StoreDto> list = jdbcTemplate.query(sql,storeLikeMapper,data);
	         return list;
	}
	//찜목록
	public StoreDto selectStoreByStoreNo(int storeNo) {
    String sql = "SELECT * FROM store WHERE store_no = ?";
    Object[] data = { storeNo };
    return jdbcTemplate.queryForObject(sql,storeMapper,data);
	}

...
<style>
    .row-clickable:hover {
        background-color: #cceeff; /* 파란색으로 하이라이팅 */
        transform: scale(1.05); /* 조금 커지는 효과 */
    }
</style>
        ...
        <table class="table table-horizontal">
            ...
            <tbody align="center">
                <c:forEach var="dto" items="${list}">
                    <tr class="row-clickable" data-storeNo="${dto.storeNo}" style="cursor: pointer;">
                        <td>${dto.storeNo}</td>
                        <td>${dto.storeName}</td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        rows.forEach(function(row) {
            row.addEventListener('click', function() {
                var storeNo = this.getAttribute('data-storeNo');
                window.location.href = 'detail?storeNo=' + storeNo;
            });
        });
    });
</script>

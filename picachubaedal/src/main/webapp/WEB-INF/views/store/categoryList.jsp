    ...
    <div class="cell">
        ...
        <div class="page-navigator">
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <a href="categoryList?page=${storeVO.getPrevBlock()}&${storeVO.getQueryString()}">&lt;이전</a>
                </c:otherwise>
            </c:choose>
            
            <%-- 페이지 숫자 --%>
            <c:forEach var="i" begin="${storeVO.getBeginBlock()}" end="${storeVO.getEndBlock()}" step="1">
                <c:choose>
                    <c:when test="${storeVO.isCurrentPage(i)}">
                        <a class="on">${i}</a>
                    </c:when>
                    <c:otherwise>
                        <a href="categoryList?page=${i}&${storeVO.getQueryString()}">${i}</a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <a href="categoryList?page=${storeVO.getNextBlock()-1}&${storeVO.getQueryString()}">다음&gt;</a> 
                </c:otherwise>
            </c:choose>
        </div>
    </div>
